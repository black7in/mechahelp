                                    ...
                                    <div class="card-body">
                                        <div class="card-use-box">
                                            <div class="crd-bx-img">
                                                <img src="{{ asset('images/buzon.png') }}" class="rounded-circle"
                                                    alt="" style="height: 100px; width: 100px;">

                                                <div class="active"></div>
                                            </div>
                                            <div class="card__text">
                                                <h4 class="mb-0">{{ $req->workshop->name }}</h4>
                                                <p>{{ $req->workshop->user->email }}</p>
                                            </div>
                                            <ul class="card__info">
                                                <li>
                                            ...
                                            <ul class="post-pos">
                                                <li>
                                                    <span class="card__info__stats">Propietario: </span>
                                                    <span>{{ $req->workshop->user->first_name }}
                                                        {{ $req->workshop->user->last_name }}</span>
                                                </li>
                                                <li>
                                                    <span class="card__info__stats">Fecha: </span>
                                                    <span>{{ $req->created_at }}</span>
                                                </li>
                                            </ul>
                                            <div>
                                                @if ($req->assistance->status < 1)
                                                    <a wire:click="aceptar({{$req->id}})" class="btn btn-primary btn-sm me-2">Aceptar</a>
                                                @endif
                                                

                                                <a data-bs-toggle="modal" data-bs-target="#{{ $req->id }}"
                                                    class="btn btn-primary btn-sm me-2">Detalles</a>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="modal-dialog" role="document">
                                    <div class="modal-content">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <ul>
                                                <li>
                                                    <strong>Precio:</strong> {{ $req->price }}
                                                </li>
                                                <li>
                                                    <strong>Tiempo de llegada</strong>: {{ $req->time }} min.
                                                </li>
                                            </ul>
                                        </div>
                                    ...
                                    <tbody>
                                        @foreach ($requests as $req)
                                            <tr>
                                                ...
                                                <td>{{ $req->workshop->user->email }}</td>
                                                <td class="text-center">175</td>
                                                <td class="text-center">12</td>
                                                <td class="text-center">100</td>
                                                <td>{{ $req->workshop->user->first_name }}
                                                    {{ $req->workshop->user->lat_name }}</td>
                                                <td>{{ $req->created_at }}</td>
                                                <td class="text-center">
                                                    <a class="btn btn-primary shadow btn-xs sharp me-1"
                                                        data-bs-toggle="modal" data-bs-target="#{{ $req->id }}">
                                                        <i class="fa fa-eye">
                                                        </i>
                                                    </a>
                                                </td>
